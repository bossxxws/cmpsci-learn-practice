...
class Album:
    ...
    def remove_songs_by_artist(self, artist):
        songs_to_remove = []
        for ele in self.song_list:
            if ele.artist == artist:
                songs_to_remove.append(ele)
        for ele in songs_to_remove:
            self.song_list.remove(ele)
            self.num -= 1
...
album.add_song(s1)
album.add_song(s2)
album.remove_songs_by_artist('Radiohead')
print(album)
