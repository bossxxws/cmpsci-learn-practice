...
def print_insert_position(bst, value):
    ...
    while node is not None:
        if value < node.get_data():
            if node.get_left() is None:
                print("To the left of", node.get_data())
                return
            else:
                node = node.get_left()
        elif value > node.get_data():
            if node.get_right() is None:
                print("To the right of", node.get_data())
                return
            else:
                node = node.get_right()
        else:
            print("Duplicate")
            return
def is_binary_search_tree(my_tree, min_value=None, max_value=None):
    if my_tree is None:
        return True
    
    if min_value is not None and my_tree.get_data() <= min_value:
        return False
    
    if max_value is not None and my_tree.get_data() >= max_value:
        return False
    
    left_is_bst = is_binary_search_tree(my_tree.get_left(), min_value, my_tree.get_data())
    right_is_bst = is_binary_search_tree(my_tree.get_right(), my_tree.get_data(), max_value)
